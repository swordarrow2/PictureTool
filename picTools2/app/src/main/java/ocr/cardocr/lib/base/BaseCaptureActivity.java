...
import ocr.cardocr.lib.bean.BankCardBean;
import ocr.cardocr.lib.bean.BankInfo;
import ocr.cardocr.lib.bean.DrivingLicenseBean;
import ocr.cardocr.lib.bean.DrivingLicenseInfo;
import ocr.cardocr.lib.bean.IDCardBackBean;
import ocr.cardocr.lib.bean.IDCardFrontBean;
import ocr.cardocr.lib.bean.IdCardInfo;
import ocr.cardocr.lib.common.CardType;
import ocr.cardocr.lib.common.YTServerAPI;
import ocr.cardocr.lib.widget.LoadingDialog;
...
public abstract class BaseCaptureActivity extends BasePermissionFragmentActivity
    ...
    protected boolean handleBankCardData(boolean isSuccessful, Bundle bundle, String responseBody) {
        ...
        if (bankCardBean.errorcode == 0) {
            List<BankCardBean.ItemsBean> items = bankCardBean.items;
            if (items != null && items.size() > 0) {
                for (BankCardBean.ItemsBean item : items) {
