...
import com.meng.qqbotandroid.MainActivity;

/**
 * QQ客户端
 */
public class QQClient extends Activity{
    ...
    public QQClient(QQChatListener qqChatListener, File folder, Handler handler) {
        ...
        this.folder = folder;
        Log.i(tag,handler.getLevel().toString());
    ...
    public void sendText(QQContact contact, String content) {
        if (contact instanceof QQUser) {
            Log.i(tag,String.format("向好友【%s】发送消息：%s", contact.name, content));
            qqAPI.send_buddy_msg2(contact.id, content);
        } else if (contact instanceof QQGroup) {
            Log.i(tag,String.format("向群【%s】发送消息：%s", contact.name, content));
            qqAPI.send_qun_msg2(contact.id, content);
        } else if (contact instanceof QQDiscuss) {
            Log.i(tag,String.format("向讨论组【%s】发送消息：%s", contact.name, content));
            qqAPI.send_discu_msg2(contact.id, content);
        } else {
            Log.i(tag,String.format("消息发送失败：联系人【%s】的类型未知", contact.name));
        }
    }
    ...
    public void changeStatus(String status) {
        Log.i(tag,String.format("改变我的在线状态为【%s】", status));
    ...
    private class QQThread extends Thread {
        ...
        public void run() {
            ...
            while (!isInterrupted()) {
                //用户登录
                Log.i(tag,String.format("正在进行第%d次登录", loginCount));
        ...
        private String initial() {
            try {
                ...
                Log.i(tag,String.format("获取到自身信息：%s", QQTools.GSON.toJson(qqContacts.me)));
                //获取好友列表
                ...
                Log.i(tag,String.format("获取到好友 %d 个", qqContacts.friends.size()));
                //获取群列表
                qqContacts.groups.putAll(qqAPI.get_group_name_list_mask2().result.convert());
                Log.i(tag,String.format("获取到群 %d 个", qqContacts.groups.size()));
                //获取和整理讨论组列表
                qqContacts.discusses.putAll(qqAPI.get_discus_list().result.convert());
                Log.i(tag,String.format("获取到讨论组 %d 个", qqContacts.discusses.size()));
                //获取在线的好友信息
                ...
                e.printStackTrace();
                Log.i(tag,String.format("初始化异常：%s\n%s", e.getMessage(), Arrays.toString(e.getStackTrace())));
            ...
            try {
                ...
                while (!isInterrupted()) {
                    try {
                        ...
                        if (rspPoll.result != null) {
                            for (ResultPoll.Item item : rspPoll.result) {
                                ...
                                try {
                                    ...
                                    e.printStackTrace();
                                    Log.i(tag,String.format("解析消息出错：%s\n%s", e.getMessage(), Arrays.toString(e.getStackTrace())));
                                    qqMessage = parseCommon(item.value, new QQUnknown());
                                }
                                if (qqMessage instanceof QQText) {
                                    if (qqMessage.fromGroup != null) {
                                        if (qqMessage.fromGroupMember.id == qqContacts.me.id) {
                                            Log.i(tag,String.format("我在群【%s】里说：%s", qqMessage.fromGroup.name, qqMessage.content));
                                        } else {
                                            Log.i(tag,String.format("群【%s】里的【%s】说：%s", qqMessage.fromGroup.name, qqMessage.fromGroupMember.name, qqMessage.content));
                                        }
                                    }
                                    if (qqMessage.fromDiscuss != null) {
                                        if (qqMessage.fromDiscussMember.id == qqContacts.me.id) {
                                            Log.i(tag,String.format("我在讨论组【%s】里说：%s", qqMessage.fromDiscuss.name, qqMessage.content));
                                        } else {
                                            Log.i(tag,String.format("讨论组【%s】里的【%s】说：%s", qqMessage.fromDiscuss.name, qqMessage.fromDiscussMember.name, qqMessage.content));
                                        }
                                    }
                                    if (qqMessage.fromUser != null) {
                                        if (qqMessage.fromUser.id == qqContacts.me.id) {
                                            Log.i(tag,String.format("我对好友【%s】说：%s", qqMessage.toUser.name, qqMessage.content));
                                        } else {
                                            Log.i(tag,String.format("好友【%s】对我说：%s", qqMessage.fromUser.name, qqMessage.content));
                                        }
                                    }
                                    qqChatListener.onMessage(qqMessage);
                                } else if (qqMessage != null) {
                                    Log.i(tag,"收到了未知类型的消息");
                                    qqChatListener.onMessage(qqMessage);
                                }
                            }
                            emptyCount = 0;
                        ...
                        e.printStackTrace();
                        if (retryCount++ < 5) {
                            Log.i(tag,String.format("监听或解析失败，重试第%d次", retryCount));
                        } else {
                            Log.i(tag,String.format("监听或解析失败：%s\n%s", e.getMessage(), Arrays.toString(e.getStackTrace())));
                            Log.i(tag,"连接已失效：监听或解析重试次数过多");
                            return LISTEN_EXCEPTION;
                        }
                    }
                }
                ...
                e.printStackTrace();
                Log.i(tag,String.format("监听异常：%s\n%s", e.getMessage(), Arrays.toString(e.getStackTrace())));
                return LISTEN_EXCEPTION;
            }
        }
