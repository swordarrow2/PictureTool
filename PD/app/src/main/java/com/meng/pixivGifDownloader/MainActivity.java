...
public class MainActivity extends AppCompatActivity{
    ...
    private final int PERMISSION_INTERNET_REQUEST_CODE = 1;
    private final int PERMISSION_MOUNT_UNMOUNT_FILESYSTEMS_REQUEST_CODE = 2;
    private final int PERMISSION_WRITE_EXTERNAL_STORAGE_REQUEST_CODE = 3;
    ...
    private void init(){
        ...
        if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.M){
            requestPermissions(Manifest.permission.INTERNET,"本程序需要网络权限");
            requestPermissions(Manifest.permission.WRITE_EXTERNAL_STORAGE,"本程序需要存储器权限");
		  }
        ...
        tmpFolder=MainActivity.sp.getValue(Data.preferenceKeys.tmpPath);
        autoInput();
    ...
    private void requestPermissions(final String permission,String Tip){
        if(ContextCompat.checkSelfPermission(this,permission)!=PackageManager.PERMISSION_GRANTED){
            if(ActivityCompat.shouldShowRequestPermissionRationale(this,permission)){
                new AlertDialog.Builder(this).setMessage(Tip).setPositiveButton("确定",new DialogInterface.OnClickListener() {
					  @Override
            ...
            case PERMISSION_INTERNET_REQUEST_CODE:
			  if(grantResults[0]==PackageManager.PERMISSION_DENIED){
				  if(!ActivityCompat.shouldShowRequestPermissionRationale(this,Manifest.permission.INTERNET)){
					  Toast.makeText(this,"网络权限已被禁止",Toast.LENGTH_SHORT).show();
                    }
                }
			  break;
